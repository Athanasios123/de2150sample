...
extern volatile int switches, c_sum;
 ...
 ******************************************************************************/
{
	...
	asm ( "stw	r23, 92(sp)");
	asm ( "stw	r25, 100(sp)");
	// r25 = bt (skip r24 = et, because it is saved above)
	asm ( "stw	r26, 104(sp)");
	// r26 = gp
...
void pushbutton_ISR(void) {
	int read_IO_button = IORD_ALTERA_AVALON_PIO_EDGE_CAP(PUSH_BUTTON13_BASE); // set as positive edge -> read only on positive egde of button this register
	if (read_IO_button & 0x2) { // reg - 0b000 0x2 - 0b0010
		switches = IORD_ALTERA_AVALON_PIO_DATA(SLIDER_SWITCHES_BASE);
		c_sum += switches;
		IOWR_ALTERA_AVALON_PIO_EDGE_CAP(PUSH_BUTTON13_BASE, 0);
		// reset the edge capture register
	}
}

void interrupt_handler(void) {
	int ipending = NIOS2_READ_IPENDING(ipending);
	if (ipending & 0x2) { // interval timer is interrupt level 1
		pushbutton_ISR();
	}
